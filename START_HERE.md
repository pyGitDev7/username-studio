...
3. `2` - проверить доступность. Система сначала берёт свежий батч, если его нет - лучшие unchecked из базы.
4. Создание канала происходит только после подтверждения:
   ```text
