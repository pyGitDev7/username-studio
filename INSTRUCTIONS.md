...
Live-проверки на вкладке **Telegram** автоматически используют аккаунты из `sessions/`. Если аккаунтов нет, остается старый режим с сессией из `.env`.
...
Telegram подключается лениво: при запуске меню соединение не создаётся. Подключение происходит только при реальной проверке или создании канала.
...
5. Откройте `2`, чтобы проверить лучшие username.
6. Если username доступен, скрипт спросит:
   ```text
...
В одиночном режиме скрипт ждёт указанное Telegram время. В мульти-аккаунт режиме аккаунт уходит в `cooldown`, а проверка переключается на следующий доступный аккаунт.
