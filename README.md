...
flowchart LR
    ...
    R -->|"Нет"| S["Одиночная сессия из .env"]
...
После успешного входа аккаунт появляется в таблице и сразу используется для live-проверок. Если в `sessions/` есть хотя бы один живой аккаунт, live-проверка на вкладке **Telegram** использует ротацию. Если аккаунтов нет, работает старый одиночный режим через `.env`.
...
| Multi-account sessions | `sessions/*.session`, `sessions/*.json` |
