...
| Мульти-аккаунты | Позволяет добавить несколько Telegram-аккаунтов и автоматически ротировать их при cooldown/errors. |
...
| Telegram API credentials | Для основного аккаунта создания каналов; live-проверки используют аккаунты из `sessions/`. |
...
|---|---|---|
| `TELEGRAM_API_ID` | Создание каналов | API ID основного аккаунта из `https://my.telegram.org/`. |
| `TELEGRAM_API_HASH` | Создание каналов | API hash основного аккаунта. Не публиковать. |
| `TELEGRAM_PHONE` | Создание каналов | Телефон основного аккаунта для Telethon-сессии. |
...
flowchart LR
    ...
    H --> R{"Есть аккаунты в sessions/?"}
    R -->|"Да"| M["Ротация аккаунтов с учетом cooldown"]
    R -->|"Нет"| S["Нужно добавить аккаунт во вкладке Аккаунты"]
    M --> I["Обновление статуса: available, checked_taken, invalid, error"]
    S --> E
    I --> J{"Создать канал?"}
    J -->|"Нет"| E
...
4. Проверить live только небольшую выбранную группу через аккаунты из вкладки **Аккаунты**.
5. Создавать канал через основной аккаунт только для подтвержденного `available` username.

## Основной Telegram-аккаунт для создания

Верхний блок на вкладке **Telegram** теперь является основным аккаунтом для создания каналов. Этот аккаунт хранится через `.env` и `telegram_session.session`, но не участвует в live-проверке доступности username.

### 1. Заполнить Telegram API

В блоке **Telegram API основного аккаунта** укажите реальные данные с `https://my.telegram.org/`:

| Поле в интерфейсе | Что вставить |
|---|---|
| `API ID` | Значение `TELEGRAM_API_ID`, это число. |
| `API HASH` | Значение `TELEGRAM_API_HASH`. |
| `Телефон` | Номер основного Telegram-аккаунта для создания каналов, например `+79990000000`. |

После заполнения нажмите **Сохранить API**. Данные будут записаны в локальный `.env`.

Если слева отображается `not configured` и текст `TELEGRAM_API_ID/TELEGRAM_API_HASH не заданы в .env`, основной аккаунт для создания каналов еще не готов: сначала нужно сохранить настоящий API ID и API HASH.

### 2. Войти по Telegram-коду

После сохранения API:

1. В блоке **Вход в основной аккаунт** проверьте телефон.
...
Создание канала доступно только после успешной авторизации основной Telethon-сессии. Live-проверка выполняется отдельно через аккаунты из вкладки **Аккаунты**.

## Мульти-аккаунты Telegram

Вкладка **Аккаунты** позволяет добавить несколько Telegram-аккаунтов без изменения `.env` и `TELEGRAM_SESSION_NAME`. Каждый аккаунт хранится локально в папке `sessions/`: session-файл Telethon и JSON с API ID/API Hash, телефоном и статусом.

Как добавить аккаунт:

1. Откройте вкладку **Аккаунты**.
2. Нажмите **+ Добавить**.
3. Введите `API ID`, `API HASH` и полный номер телефона.
4. Нажмите **Отправить код**.
5. Введите код из Telegram прямо в интерфейсе.
6. Если Telegram попросит 2FA, появится поле для пароля.

После успешного входа аккаунт появляется в таблице и сразу используется для live-проверок. Если в `sessions/` нет активного аккаунта, live-проверка не запускается. Основной аккаунт из `.env` для проверки доступности не используется.

Статусы аккаунтов:

| Статус | Значение |
|---|---|
| `active` | Аккаунт доступен для проверки. |
| `cooldown` | Аккаунт временно пропускается после FloodWait/RPCError. |
| `dead` | Сессия больше не авторизована или получила auth/session error. |

При `FloodWaitError` аккаунт уходит в cooldown на время из Telegram. При обычном `RPCError` cooldown равен 120 секундам. Ошибки авторизации помечают аккаунт как `dead`. Текущий активный аккаунт показывается на вкладке **Telegram** полным номером телефона.
...
| Выключен | Будет реальный Telegram-запрос через аккаунты из вкладки **Аккаунты**. |

Для live-проверки нужно:

1. Убедиться, что во вкладке **Аккаунты** есть активный аккаунт для проверки.
2. Снять галочку **dry-run**.
3. В поле **Live confirm** написать ровно:
   ```text
   CHECK
   ```
4. Нажать **Проверить выбранные live**.
...
| Web host | `127.0.0.1` |
| Web port | `8080` |
| Основной аккаунт создания | `telegram_session.session` |
| Аккаунты live-проверки | `sessions/*.session`, `sessions/*.json` |
